...
The app is already online, hosted free on GitHub Pages:

```
https://enislucas.github.io/quran-player/quran/
```

To publish a change: commit, bump `SHELL_CACHE` in `sw.js` (this is what makes
phones pick the update up), and `git push`. GitHub redeploys in a couple of
minutes. The new version appears the next time she opens the app — it will not
reload itself mid-recitation.

Note: every individual file here is under GitHub's 100 MB per-file limit, and the whole site fits comfortably under GitHub Pages' roughly 1 GB size limit. Don't add other large files to the repo, or it may go over that limit.

## Setting up grandma's phone

**Android (Chrome):**
1. Open https://enislucas.github.io/quran-player/quran/ in Chrome.
2. Tap the **⋮** menu in the top corner.
3. Tap **"Add to Home screen"** (sometimes shown as **"Install app"**).

**iPhone (Safari):**
1. Open https://enislucas.github.io/quran-player/quran/ in Safari.
